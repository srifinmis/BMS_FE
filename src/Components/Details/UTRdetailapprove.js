...
const fieldConfig = [
    ...
    { name: "due_date", label: "Actual Due-Date", type: "date" },
    { name: "payment_date", label: "Payment Date", type: "date" },
    { name: "payment_amount", label: "Payment Amount", type: "number" },
    { name: "utr_no", label: "UTR NO.", type: "number" },
];

const UTRapprove = ({ isDropped }) => {
    ...
    const handleApprove = async () => {
        try {
            const response = await fetch(`${API_URL}/UTR/Approve`, {
                method: "POST",
                headers: {
            ...
            if (!response.ok) {
                ...
                localStorage.setItem("submissionMessage", "UTR Upload Approve failed!");
    ...
    const handleReject = async () => {
        ...
        try {
            const response = await fetch(`${API_URL}/UTR/reject`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            if (!response.ok) {
                const errorData = await response.json();
                console.error("Server responded with error:", errorData);
                localStorage.setItem("submissionMessage", "UTR Upload Rejection failed!");
                localStorage.setItem("messageType", "error");
                throw new Error(`Server Error: ${response.status}`);
            }
    ...
    return (
        ...
        >
            ...
            {loading ? (
                ...
                <Typography sx={{ textAlign: "center", marginTop: 2 }} >UTR Upload details not found</Typography>
            )}
        </Box>
    );
};

export default UTRapprove;